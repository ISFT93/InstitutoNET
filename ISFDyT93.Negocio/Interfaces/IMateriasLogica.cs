...
namespace ISFDyT93.Negocio.Interfaces
{
    public interface IMateriasLogica
    {
       ...
       // int AgregarMaterias(MateriasModelo modelo);
        ...
        int AgregarMateria(MateriasModelo modelo, int anioCarreraId);
        int CalcularModulos(int cargaHoraria);
        int GuardarMateria(MateriasModelo modelo, TipoAccion accion, int anioCarreraId);
        string[] ObtenerNombresMateriasLista();
    }
}
